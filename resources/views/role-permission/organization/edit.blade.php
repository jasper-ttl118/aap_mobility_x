...
<body class="flex flex-row h-screen">
    ...
    <div class="flex flex-col w-full ml-64 overflow-y-auto p-10 h-screen justify-center items-center">
        ...
        @endif
        <form action="{{ url('organization/'.$organization->org_id) }}" method="post" enctype="multipart/form-data" class="max-w-lg mx-auto bg-white shadow-md rounded-lg overflow-hidden border border-gray-200">
            @csrf
            @method('PUT')
            <div class="p-6 space-y-6">
                <!-- Heading -->
                <div class="border-b border-gray-200 pb-5">
                    <h1 class="text-xl font-semibold text-gray-800">Update New Organization</h1>
                    <p class="text-sm text-gray-500">
                        Update organization details for managing user access and roles.
                    </p>
                </div>
                ...
                <div>
                    ...
                    <input type="text" value="{{ $organization->org_name }}" name="org_name" placeholder="Organization Name"
                ...
                <div>
                    ...
                    <input type="text" value="{{ $organization->org_description }}" name="org_description" placeholder="Description"
                        class="w-full bg-gray-50 border border-gray-300 rounded-md px-4 py-2 mt-1 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                </div>
                ...
                <div class="grid grid-cols-2 gap-2">
                    ...
                    <select name="org_status" class="w-full bg-gray-100 h-10 rounded border border-gray-300 px-3 mt-1 focus:outline-blue-500">
                        <option value='1' @selected(intval(old('org_status', $organization->org_status)) === 1)>Active</option>
                        <option value='0' @selected(intval(old('org_status', $organization->org_status)) === 0)>Inactive</option>
                    </select>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-4">
                    <div class="shrink-0">
                        <img id='preview_img' class="h-40 w-40 object-cover rounded-md" src="{{ asset('storage/'.$organization->org_logo) }}" alt="Current profile photo" />
                    </div>
