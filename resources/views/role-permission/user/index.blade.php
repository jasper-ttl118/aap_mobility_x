            ...
            <div class="w-full m-10 rounded-md border bg-white border-gray-100 p-5 shadow shadow-gray-300">
                <div class="flex justify-between items-center p-5">
                    ...
                    @can("User Management: Add")
                        <a href="user/create" class="flex uppercase rounded-lg bg-blue-900 p-3 text-xs font-bold text-white hover:bg-indigo-800">
                ...
                <table class="w-full text-center text-sm text-gray-500">
                    ...
                    <tr>
                        ...
                        @if (auth()->user()->can('User Management: Edit') || auth()->user()->can('User Management: Delete'))
                    ...
                    <tr class="border-b border-gray-200 bg-white">
                        <th scope="row" class="px-6 py-4 font-medium whitespace-nowrap text-gray-900">{{ $user->id }}</th>
                        <td class="px-6 py-4 text-gray-900">{{ $user->name }}</td>
                        <td class="px-6 py-4  text-gray-900">{{ $user->email }}</td>
                        <td class="px-6 py-4">
                        @if($user->roles->isNotEmpty())
                            @foreach($user->roles as $role)
                                <span class="bg-blue-600 whitespace-nowrap text-white text-xs font-medium px-2 py-1 rounded-full">{{ $role->name }}</span>
                            @endforeach
                        @else
                            <span class="bg-gray-500 whitespace-nowrap text-white text-xs font-medium px-2 py-1 rounded-full">No Role Assigned</span>
                        @endif
                        </td>
                        @if (auth()->user()->can('User Management: Edit') || auth()->user()->can('delete posts'))
                        <td class="px-6 py-4 flex flex-wrap gap-3 justify-center">
                            @can("User Management: Edit")
                                <a href="{{ url('user/'.$user->id.'/edit') }}" class="flex items-center gap-1 font-medium text-blue-800 underline">
                            ...
                            @can("User Management: Delete")
                                <a href="javascript:void(0)" onclick="openModal('{{ url('user/'.$user->id) }}')" class="flex items-center gap-1 font-medium text-red-700 underline">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-4">
        ...
        <div id="delete-modal" class="fixed inset-0 z-50 flex items-center justify-center hidden bg-gray-900 bg-opacity-50">
            <div class="bg-white rounded-lg shadow-lg w-96 p-6 dark:bg-gray-800">
                ...
                <div class="mt-4 flex justify-end space-x-3">
                    <button onclick="closeModal()" class="px-4 py-2 text-gray-700 bg-gray-200 rounded-lg hover:bg-gray-300 dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600">
                        Cancel
                    </button>
