...
<head>
    ...
    <body class="flex flex-row h-screen">
        ...
        <div class="flex flex-col w-full ml-64 overflow-y-auto p-2 h-screen">
            @if ($errors->any())
            ...
            @endif
            <form action="{{ url('user/'.$user->user_id) }}" method="post" class="w-full">
                @csrf
                @method('PUT')
                <div class="text-gray-700 m-20 flex flex-col gap-2">
                    <div class="flex flex-col pb-5 mb-5 border-b-2 border-gray-200">
                        <h1 class="text-lg font-bold uppercase">Edit user account for <span class="text-blue-800">{{$employee->employee_firstname}} {{$employee->employee_lastname}}</span></h1>
                        <p class="text-sm">Update user details.</p>
                    </div>
                    ...
                    <div class="mt-4">
                        ...
                        <input type="text" value="{{ $user->user_name }}" name="user_name" placeholder="Enter unique username" class="w-full bg-gray-100 h-10 rounded border border-gray-300 px-3 mt-1 focus:outline-blue-500">
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        <div>
                        <label for="user_status" class="font-medium">Set Status</label>
                        <select name="user_status" class="w-full bg-gray-100 h-10 rounded border border-gray-300 px-3 mt-1 focus:outline-blue-500">
                            <option value='1' @selected(intval(old('user_status', $user->user_status)) === 1)>Active</option>
                            <option value='0' @selected(intval(old('user_status', $user->user_status)) === 0)>Inactive</option>
                        </select>
                        </div>
                    </div>
