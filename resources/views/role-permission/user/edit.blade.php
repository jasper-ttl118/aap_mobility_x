...
<body class="flex flex-row">
    ...
    <div class="flex flex-col items-center justify-center w-full h-auto">
        ...
        @endif
        <form form action="{{ url('user/'.$user->id) }}" method="post">
            @csrf
            @method('PUT')
            <div class="text-gray-700 m-20">
                <div class="flex flex-col pb-5 mb-5 border-b-2 border-gray-200">
                    <h1 class="text-lg font-bold uppercase">Update User Information</h1>
                    <p class="text-sm">Modify and update user details and profile information.</p>
                </div>
                <div class="flex flex-col gap-7">
                    <div class="flex flex-row gap-3">
                        <div class="flex flex-col gap-2 w-96">
                            <label for="" class="font-medium">Name</label>
                            <input type="text" name="name" value="{{ $user->name }}" placeholder="Enter Your Name" class="bg-gray-100 h-10 rounded-lg border-1 border-gray-200 px-3 focus:outline-blue-900 ">
                        </div>
                        <div class="flex flex-col gap-2 w-96">
                            <label for="" class="font-medium">Email</label>
                            <input type="email"  name="email" value="{{ $user->email }}" placeholder="sample@email.come" class="bg-gray-100 h-10 rounded-lg border-1 border-gray-200 px-3 focus:outline-blue-900 ">
                        </div>
                    </div>
                    <div class="flex flex-col gap-2 w-96">
                        <label for="" class="font-medium">Password</label>
                        <input type="password" name="password" placeholder="Enter password"class="bg-gray-100 h-10 rounded-lg border-1 border-gray-200 px-3 focus:outline-blue-900 ">
                    </div>
                    <div class="flex flex-col gap-2 w-96">
                        <label for="" class="font-medium">Role</label>
                        <select type="text" name="roles[]" placeholder="Enter Your Name"class="bg-gray-100 h-10 rounded-lg border-1 border-gray-200 px-3 focus:outline-blue-900">
