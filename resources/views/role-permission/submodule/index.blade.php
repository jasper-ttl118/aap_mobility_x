...
{{-- <body class="flex flex-row h-screen" x-data="{ open: false, deleteUrl: '', viewOpen: false, employee: {} }">
    ...
    <div class="flex flex-col w-full ml-64 overflow-y-auto p-10 h-screen items-center bg-[url('/public/build/assets/bgdiv.jpg')] bg-cover bg-center">
        @if (session('status'))
        ...
        <div class="w-full m-10 rounded-md border bg-white border-gray-100 p-5 shadow shadow-gray-300">
            <div class="flex justify-between items-center p-5">
                <h1 class="text-2xl font-bold">List of All Sub-Modules</h1>
                <a href="submodule/create" class="flex uppercase rounded-lg items-center bg-blue-900 p-3 text-xs font-bold text-white hover:bg-indigo-800">
            ...
            <table class="w-full text-center text-sm text-gray-500">
                ...
                <tr>
                    ...
                    <th scope="col" class="px-auto py-3">Assigned to</th>
                ...
                <tr class="border-b border-gray-200 bg-white">
                    <th scope="row" class="px-6 py-4 font-medium whitespace-nowrap text-gray-900">{{ $submodule->submodule_id }}</th>
                    <td class="px-auto py-4  text-gray-900">{{ $submodule->submodule_name }}</td>
                    <td class="px-auto py-4  text-gray-900">{{ $submodule->submodule_description }}</td>
                    ...
                    <td class="px-auto py-4  text-gray-900">
                        @if ($submodule->submodule_status == '1')
                    ...
                    <td class="px-auto py-4">
                        <div class="flex flex-row justify-center items-center gap-2">
                            <a href="{{ url('submodule/'.$submodule->submodule_id.'/edit') }}" class="flex items-center gap-1 font-medium text-blue-800">
                            ...
                            <a href="javascript:void(0)" @click="open = true; deleteUrl = '{{ url('submodule/'.$submodule->submodule_id.'/delete') }}'" class="flex items-center gap-1 font-medium text-red-700">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-4">
...
<body class="flex flex-row w-h-screen" x-data="{ open: false, deleteUrl: '', viewOpen: false, employee: {} }">
    @php
        ...
        <div class=" rounded-md border-2 border-gray-100 bg-gray-50">
            ...
            <div class="flex items-center justify-between p-7">
                <div>
                    ...
                    <p class="text-gray-900 text-sm">Assign and Manage submodules.</p>
                </div>
                ...
                <table class="w-full text-center text-sm text-gray-500">
                    <thead class="bg-gray-100 text-xs text-gray-700 uppercase">
                        <tr>
                            ...
                            <th scope="col" class="w-2/12 py-3">Name</th>
                            <th scope="col" class="w-4/12 py-3">Description</th>
                            <th scope="col" class="w-2/12 py-3">Assigned to</th>
                        ...
                        @foreach ($submodules as $submodule)
                            <tr class="border-b border-gray-200 bg-white">
                                <th scope="row" class="px-6 py-4 font-medium whitespace-nowrap text-gray-900">
                                    {{ $submodule->submodule_id }}</th>
                                <td class="px-auto py-4  text-gray-900">{{ $submodule->submodule_name }}</td>
                                <td class="px-auto py-4  text-gray-500">{{ $submodule->submodule_description }}</td>
                                <td class="px-auto py-4 flex flex-wrap gap-2 justify-center text-gray-900">
                                    @if ($submodule->module)
                                ...
                                <td class="px-auto py-4">
                                    <div class="flex flex-row justify-center items-center gap-2">
                                        <a href="{{ url('submodule/' . $submodule->submodule_id . '/edit') }}"
                                        ...
                                        <a href="javascript:void(0)"
                                            @click="open = true; deleteUrl = '{{ url('submodule/' . $submodule->submodule_id . '/delete') }}'"
