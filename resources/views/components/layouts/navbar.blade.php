  ...
  <div class="w-full text-xs">
    ...
    @foreach($modules as $module)
      @canAccessModule($module)
        <div class="group cursor-pointer flex items-center px-2 py-3 gap-2 ml-2 rounded-l-xl {{ $module->module_name === $navbar_selected ? 'bg-[#F3F4F6] text-blue-900 font-medium rounded-1 back' : 'hover:text-[#F6D400]' }}">
          ...
          <a href="{{ $links[$module->module_name] }}">{{ $module->module_name }}</a>
        </div>
      @endcanAccessModule
    @endforeach
  </div>
