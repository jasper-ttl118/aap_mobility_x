<!-- Modal overlay -->
<div x-show="edit_draft" x-transition class="fixed inset-0 z-50 flex items-center justify-center">
  <!-- Modal content -->
  <div @click.outside="edit_draft=false" 
        class="relative flex flex-col bg-white rounded-b-lg rounded-t-xl shadow-lg w-[80%] max-w-4xl pb-6 gap-y-4">

    <div class="flex justify-between rounded-t-lg p-3 px-5 items-center bg-[#D17E0E]">
        <h1 class="text-white text-sm">Drafted Message</h1>
        <button @click="edit_draft=false" class="text-white hover:text-gray-800 text-xl font-bold">&times;</button>
    </div>

    <div class="flex justify-center">
        <div class="flex w-[96%] h-10 items-center">
            <span class="h-full p-2 text-gray-400 text-sm border-b-2 border-gray-400">To:</span>
            <input wire:model="recipient" type="text" class="h-full w-full border-0 border-b-2 p-2 pt-1 border-gray-400" value="Everyone">
       </div>
    </div>
    ...
    <div class="flex justify-center">
        <div class="flex w-[96%] h-10 items-center">
            ...
            <input wire:model="subject" type="text" class="h-full w-full border-0 border-b-2 p-2 pt-1 border-gray-400" value="Message Template 1">
       </div>
    </div>
    ...
    <div class="flex flex-col lg:flex-row w-full h-[300px] lg:h-full justify-evenly px-4 gap-4 overflow-x-auto hide-scrollbar">
        <div class="flex justify-center items-center lg:w-[70%] lg:h-[272px] w-full h-[70%] mt-8 lg:mt-0">
            ...
            <div class=" w-full h-[252px] bg-[#EAE8E8] p-4 rounded-lg  flex flex-col justify-between ">
                ...
                <textarea wire:model="content" id="message" rows="4" class="overflow-auto hide-scrollbar resize-none h-[200px] block p-4 w-full text-sm text-gray-900 border-0 bg-gray-100 rounded-lg dark:bg-gray-300 dark:placeholder-gray-600" placeholder="Enter Your Message...">💥 A Special Surprise Just for You, [Customer Name]! 💥
...
Happy shopping! 🛍️
        ...
        <div class="flex flex-col lg:w-[30%] lg:h-full justify-evenly items-center gap-3 w-full h-[30%] mt-3 lg:mt-0">
            ...
            <div class="flex justify-end w-full gap-x-4">
                <button wire:click="update" @click="edit_draft=false" class="border-2 bg-[#071d49] p-2 rounded-lg text-white w-[50%] font-semibold">
                    Update
                </button>
                <button @click="edit_draft=false" class="border-2 bg-[#605E5E] p-2 rounded-lg text-white w-[50%] font-semibold">
                    Cancel  
                </button>
            </div>
        </div>
