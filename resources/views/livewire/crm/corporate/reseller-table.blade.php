<div x-show="corporate === 'reseller'" x-cloak>
<table class="w-full text-sm md:justify-center text-gray-500">   
    <thead class="gap-5 bg-gray-100 text-xs text-gray-700 uppercase w-[440px] lg:w-full">
        <tr>    
            <th scope="col" class="w-[6.25%] py-3">Resellers ID</th>
        ...
        @foreach ($corporates as $customer)
            <tr class="border-b border-gray-200 bg-white gap-5">
                ...
                {{-- {{$customer->customer_id}} --}}
                        <a @click="open=true" class="flex items-center gap-1 font-normal text-[#151847] cursor-pointer" >
...
<div x-show="open" x-cloak x-transition class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
        <div @click.away="open = false">
            <livewire:crm.corporate.resellers-profile customer_id="2"/>
        </div>
</div>
