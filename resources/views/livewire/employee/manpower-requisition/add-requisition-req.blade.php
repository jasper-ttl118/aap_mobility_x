<div @close-modal.window="open_add=false" 
    ...
    x-data="{ selectedStep: 0, employment_type : '', selected : 'job information', selected_tabs : ['job information', 'hiring specifications', 'requested', 'endorsed', 'approved']}"
    ...
    <form wire:submit="add" >
        ...
        <div class="text-gray-700 p-7 space-y-5 ">
            ...
            <div x-show="selected === 'job information'" class="flex flex-col gap-y-5">
                    ...
                    <div class="grid grid-cols-2 gap-5">
                        ...
                        <div>
                            ...
                            <input type="date" name="requisition_job_position" placeholder="e.g. Junior Mobile Developer"
                                wire:model="requisition_job_position"
                                class="w-full bg-gray-100 h-8 rounded border border-gray-300 px-2 text-sm mt-1 focus:outline-blue-500">
                            @error('requisition_job_position') <em class="text-sm text-red-500">{{ $message }}</em> @enderror
                        </div>
                    </div>
                                ...
                                x-data="{
                                    ...
                                    availableOptions(index) {
                                        return this.allCandidates.filter(candidate =>
                                            !this.selected.map(c => c.value).includes(candidate) || this.selected[index].value === candidate
                                        );
                                    }
                                }"
                                x-init="if (!Array.isArray(selected) || selected.length === 0) {
                                ...
                                <template x-for="(candidate, index) in selected" :key="candidate.id">
                                    <div class="flex gap-2 items-center">
                                        ...
                                        >
                                            ...
                                            <template x-for="option in availableOptions(index)" :key="option">
            ...
            <!-- Submit Button -->
            <div x-show="selected === 'requested'" class="flex gap-x-5">
                <button type="submit"
